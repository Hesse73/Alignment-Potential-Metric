...
def is_wandb_available(): return True
...
class InferSimPOTrainer(Trainer):
    r"""
    ...
    def __init__(
        ...
        args: Optional[InferSimPOConfig] = None,
        ...
        # NOTE: save additional args
        self.is_infer = args.is_infer
        self._infer_dataset = defaultdict(list) if self.is_infer else None
        if self.is_infer:
            warnings.warn("Using inference mode for logps dataset construction.")
    ...
    def concatenated_forward(
        ...
        # NOTE: inference dataset for logps
        if self.is_infer:
            # un-normalized logps
            all_logps_unnorm = self.get_batch_logps(
                all_logits,
                concatenated_batch["concatenated_labels"],
            ...
            chosen_logps_unnorm = all_logps_unnorm[:len_chosen]
            rejected_logps_unnorm = all_logps_unnorm[len_chosen:]
            
            # gather and save batch data
            for key in ['idx', 'raw_prompt', 'raw_chosen', 'raw_rejected', 'rm_margin']:
                if key not in batch: warnings.warn(f"Key {key} not in batch!"); continue
                val = self.accelerator.gather_for_metrics(batch[key])
                self._infer_dataset[key] += val

            # gather and save logps
            self._infer_dataset['chosen_logps'] += self.accelerator.gather_for_metrics(chosen_logps_unnorm.tolist())
            self._infer_dataset['rejected_logps'] += self.accelerator.gather_for_metrics(rejected_logps_unnorm.tolist())
            self._infer_dataset['chosen_logps_norm'] += self.accelerator.gather_for_metrics(chosen_logps.tolist())
            self._infer_dataset['rejected_logps_norm'] += self.accelerator.gather_for_metrics(rejected_logps.tolist())
